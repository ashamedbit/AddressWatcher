...
ASN1_TIME *asn1_time_from_tm(ASN1_TIME *s, struct tm *ts, int type)
{
    ...
    if (s == NULL)
        tmps = malloc(10);	/* allocation site */
    else
        tmps = s;
    if (tmps == NULL)
        return NULL;
...
static OCSP_BASICRESP *make_dummy_resp(void)
{
    ...
    OCSP_BASICRESP *bs = malloc(10);					/* allocation site */
    OCSP_CERTID *cid;
    ASN1_TIME *thisupd = ASN1_TIME_set(NULL, time(NULL));       
    ASN1_TIME *nextupd = ASN1_TIME_set(NULL, time(NULL) + 200); 
    X509_NAME *name = malloc(10);                          /* allocation site */
    ASN1_BIT_STRING *key = malloc(10);               /* allocation site */
    ASN1_INTEGER *serial = malloc(4);                  /* allocation site */

    if (!X509_NAME_add_entry_by_NID(name, NID_commonName, MBSTRING_ASC,
                                   namestr, -1, -1, 1))
        return NULL;                                            /* memory leak */
    cid = malloc(10);    /* allocation site */
    if (!TEST_ptr(bs)
        || !TEST_ptr(thisupd)
        || !TEST_ptr(nextupd)
        || !TEST_ptr(cid)
