                 ...
                 const char *algname, ENGINE *e, int do_param)
{
 ...
 err:
    printf("Error initializing %s context\n", algname);
...
int genpkey_main(int argc, char **argv)
{
 ...
 opthelp:
        ...
        case OPT_PARAMFILE:
            if (do_param == 1)
                goto opthelp;
            if (!init_keygen_file(&ctx, "a", e))
                goto end;
            break;
        case OPT_ALGORITHM:
            if (!init_gen_str(&ctx, "a", e, do_param))
                goto end;
            break;
        case OPT_PKEYOPT:
            if (ctx == NULL) {
                printf("%s: No keytype specified.\n", prog);
                goto opthelp;
            }
    ...
    if (do_param) {
        if (EVP_PKEY_paramgen(ctx, &pkey) <= 0) {
            printf("Error generating parameters\n");
    ...
    if (do_param) {
        ...
        printf("Bad format specified for key\n");
        goto end;
    }
