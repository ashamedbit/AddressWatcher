...
#define MAX_SI_TYPES 100
    ...
    while (1) {
        ...
        if ( ++prot_opt > 1) {
            printf("Cannot supply multiple protocol flags\n");
            goto end;
        }
 ...
 opthelp:
    ...
    if (count4or6 >= 2) {
        printf("%s: Can't use both -4 and -6\n", prog);
        goto opthelp;
    }
    if (noservername) {
        if (servername != NULL) {
    ...
    if (sess_in != NULL) {
        ...
        if (!noservername && servername == NULL) {
            ...
            if (sni != NULL) {
                servername = strdup(sni); /* allocation site */
                if (servername == NULL) {
                    printf("Can't set server name\n");
                    goto end;
                }
            } else {
                /*
    ...
    if (!noservername && (servername != NULL || dane_tlsa_domain == NULL)) {
        ...
        if (!SSL_set_tlsext_host_name(con, servername)) {
            printf("Unable to set TLS servername extension.\n");
            goto end;
        }
    }
 re_start:
